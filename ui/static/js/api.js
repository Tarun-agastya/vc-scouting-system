...
export const api = {
  ...
  /* Field-level merge (Phase 3). The preview marks which fields actually
     differ so the screen can collapse the ones both records agree on — on a
     real pair that is usually most of them. */
  mergePreview: (id) => get(`/reviews/${id}/merge-preview`),
  mergeReview: (id, choices) => post(`/reviews/${id}/merge`, { choices }),
  recentMerges: (limit = 30) => get("/reviews/merges/recent", { limit }),
  undoFieldMerge: (snapshotId) => post(`/reviews/merges/${snapshotId}/undo`),
